  ...
  const ModeToggle = (
    <div className="flex bg-surface-200/60 p-1 rounded-xl border border-white/5 shadow-inner">
      <button
        ...
        className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-colors duration-300 ${mode === 'vasos' ? 'bg-sky-500/20 text-sky-300 shadow-[0_0_12px_rgba(56,189,248,0.2)]' : 'text-white/40 hover:text-white/70'}`}
      >Vasos</button>
      <button
        onClick={() => setMode('litros')}
        className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-colors ${mode === 'litros' ? 'bg-sky-500/20 text-sky-300 shadow-[0_0_12px_rgba(56,189,248,0.2)]' : 'text-white/40 hover:text-white/70'}`}
      >Litros</button>
    </div>
  )

  const ProgressBar = (
    <div className={`w-full ${isHorizontal ? 'mb-2' : 'mb-4'}`}>
              ...
              {Array.from({ length: TARGET }, (_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`relative overflow-hidden aspect-[3/4] rounded-[20px] flex items-center justify-center transition-colors duration-300 border-2 active:scale-90 ${
                      filled
                  ...
                  >
                    ...
                    <Droplet
                      size={32}
                      className={`relative z-10 transition-colors duration-300 ${filled ? 'text-sky-300 fill-sky-400/20 drop-shadow-[0_0_8px_rgba(56,189,248,0.8)]' : 'text-white/15'}`}
                    />
                  </button>
                )
              })}
            ...
            <div className="flex flex-col items-center justify-center gap-5">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="w-10 h-10 rounded-2xl bg-sky-400/10 border border-sky-400/20 flex items-center justify-center text-sky-400/60 hover:bg-sky-400/20 hover:text-sky-400 active:scale-95 transition-colors"
                >
                  <Minus size={15} />
                </button>
                ...
                {L_PRESETS.map(l => (
                  <button
                    ...
                    className="px-3 h-10 rounded-xl text-[11px] font-black text-sky-400/60 hover:text-sky-300 bg-sky-400/5 hover:bg-sky-400/15 border border-sky-400/10 hover:border-sky-400/25 transition-colors active:scale-90"
                  >
                    +{l}L
                  </button>
                ))}
            ...
            <div className="flex items-center gap-2">
              {L_PRESETS.map(l => (
                <button
                  key={l}
                  onClick={() => onChange(Math.round((liters + l) * 1000))}
                  className="px-3 h-9 rounded-xl text-[11px] font-black text-sky-400/60 hover:text-sky-300 bg-sky-400/5 hover:bg-sky-400/15 border border-sky-400/10 hover:border-sky-400/25 transition-colors active:scale-90"
                >
                  +{l}L
                </button>
              ))}
